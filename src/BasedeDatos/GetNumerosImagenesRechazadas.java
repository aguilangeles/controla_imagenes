...
//
    ...
    if (c.isConexion()) {
      try {
        String query = "SELECT count(estado) FROM qualitys.archivo where idtraza = " + idTraza + " and estado = 1;";
        c.executeQuery(query);
        while (c.resulset.next()) {
          numero = c.resulset.getInt(1);
        }
        String update = "UPDATE `qualitys`.`traza` SET `nro_rechazo` = " + numero + " WHERE id = " + idTraza + ";";
