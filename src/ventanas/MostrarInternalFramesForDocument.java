...
public class MostrarInternalFramesForDocument {
    ...
    private void setTituloYRutaLabel(Imagen imagen, int cantSublote) {
        rutasublote = imagen.getRutaParaConversion();
        String sublote = getRutaSublote() + " (" + cantSublote + "/ " + totalDocumento + ")";
        internal.setTitle(sublote);
        rutaLabel.setText(imagen.getRutaInsertadaEnDB());
    }

    private String getRutaSublote() {
        String ret = rutasublote.replace("\\\\", File.separator);
        return ret;
    }
    ...
    private void adaptarFrame(Imagen imagen, int nroImagen, Sublote sublote, int nroSublote) {
        setTituloYRutaLabel(imagen, nroSublote);
